<template>
  <div class="flex flex-col h-screen overflow-hidden font-sans" style="background-color: var(--color-bg); color: var(--color-text);">
    ...
    <div
      ...
      class="shrink-0 flex items-center gap-2 px-4 py-1.5 border-b bg-amber-50 dark:bg-amber-950/30 border-amber-300 dark:border-amber-900 text-xs z-30"
    >
      <Crown class="w-3.5 h-3.5 text-amber-700 dark:text-amber-400" />
      <span class="font-bold text-amber-900 dark:text-amber-300">Super Admin — Viewing School:</span>
      <div class="relative flex items-center">
        <select
          v-model="selectedTenant"
          @change="handleTenantChange"
          class="bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-bold text-xs pl-2 pr-6 py-0.5 rounded-sm border border-amber-300 dark:border-amber-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-500 cursor-pointer appearance-none"
        >
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden relative">
      ...
      <header
        ...
        class="h-14 flex items-center justify-between px-6 border-b z-20 shrink-0"
        style="background-color: var(--color-header-bg); border-color: var(--color-header-border);"
      >
        <!-- Breadcrumb Navigation -->
        <div class="flex items-center gap-2">
          <router-link to="/" class="text-xs font-semibold theme-text-muted hover:text-slate-900 dark:hover:text-slate-100 transition-colors">Workspace</router-link>
          <span class="text-slate-300 dark:text-slate-700 text-xs">/</span>
          <span class="text-xs font-bold capitalize theme-text-heading">{{ $route.name ? String($route.name).replace(/-/g, ' ') : 'Dashboard' }}</span>
        </div>
        ...
        <div v-if="authStore.user" class="flex items-center gap-2.5">
          ...
          <router-link
            ...
            to="/manage/users"
            class="px-3 py-1.5 rounded-sm bg-blue-600 hover:bg-blue-700 text-white font-semibold text-xs transition-colors flex items-center gap-1.5 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-1"
          ...
          <div class="hidden sm:flex items-center gap-2 px-3 py-1 border rounded-sm text-xs" style="background-color: var(--color-surface-subtle); border-color: var(--color-border);">
            <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 shrink-0"></span>
            <span class="theme-text-muted font-medium">{{ authStore.user.email }}</span>
            <span class="px-1.5 py-0.5 rounded-sm text-[10px] font-bold uppercase tracking-wider bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-300 dark:border-slate-700">
          ...
          <button
            @click="toggleTheme"
            class="p-1.5 rounded-sm border transition-colors flex items-center gap-1.5 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          ...
          <router-link
            to="/profile"
            class="h-8 w-8 rounded-sm bg-slate-900 dark:bg-blue-600 text-white flex items-center justify-center font-bold text-xs shrink-0 transition-all hover:ring-2 hover:ring-blue-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
            title="My Account"
          >
          ...
          <button
            id="logout-btn"
            @click="handleLogout"
            class="p-1.5 rounded-sm border transition-colors flex items-center gap-1.5 text-xs font-semibold text-rose-600 dark:text-rose-400 border-rose-200 dark:border-rose-900 hover:bg-rose-50 dark:hover:bg-rose-950/40 cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-500"
            title="Sign Out"
          >
      ...
      <!-- Page content: full width, no artificial center-column cap here --
           individual views own their own max-width where content genuinely
           benefits from one (e.g. a single form), but the shell itself never
           narrows the canvas. -->
      <main class="flex-1 overflow-y-auto relative z-10 px-4 py-4 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
